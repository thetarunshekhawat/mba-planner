...
## Admin access
Admins are hardcoded in `ADMIN_EMAILS` (a `Set<string>`) at the top of both `app/admin/page.tsx` and `app/planner/page.tsx`. All emails must be lowercase.
...
- When adding a new admin, update `ADMIN_EMAILS` in **both** files and this list.
